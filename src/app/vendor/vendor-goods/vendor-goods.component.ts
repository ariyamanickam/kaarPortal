...
@Component({
  ...
  styleUrls: ['./vendor-goods.component.scss']
})
export class VendorGoodsComponent implements OnInit {
  ...
  public displayedColumns: string[] = ['MAT_DOC', 'PSTNG_DATE', 'DOC_DATE', 'DOC_YEAR'];
  ...
  ngOnInit(): void {
    ...
    this.vendorService.getGoods().subscribe((data) => {
      ...
      this.goodslistresults = this.goodslist['SOAP:Envelope']['SOAP:Body'][0]['ns0:ZBAPI_MM_GOODSLIST_AM.Response'][0]['IT_GOODSMVT_HEADER'][0]['item']
      console.log("Goods",this.goodslistresults);
      this.dataSource = new MatTableDataSource(this.goodslistresults);
  ...
  fragment(data: any) {
    ...
    this.vendorService.PostGoodsdetail(newGoods).subscribe((data) => {
      console.log(data);
      

    }, (err) => console.log(err));
    this.dialog.open(VendorGoodsDetailComponent,{
