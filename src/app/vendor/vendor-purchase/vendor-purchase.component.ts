...
@Component({
  ...
  templateUrl: './vendor-purchase.component.html',
  styleUrls: ['./vendor-purchase.component.scss']
})
export class VendorPurchaseComponent implements OnInit {
  ...
  fileName= 'Vendor_Purchase.xlsx';

  purchaselist : any;
  purchaselistresults: any;

  public dataSource: MatTableDataSource<any>;
  public displayedColumns: string[] = ['PO_NUMBER', 'PURCH_ORG', 'PUR_GROUP', 'CUSTOMER','DOC_DATE','EXPORT_NO','DOC_COND'];
  ...
  ngOnInit(): void {
    ...
    this.vendorService.getPurchase().subscribe((data) => {
      ...
      this.purchaselistresults = this.purchaselist['SOAP:Envelope']['SOAP:Body'][0]['rfc:ZBAPI_MM_PURCHASEORD_LIS_AM.Response'][0]['IT_PO_HEADERS'][0]['item']
      console.log("Purchase",this.purchaselistresults);
      this.dataSource = new MatTableDataSource(this.purchaselistresults);
  ...
  fragment(data: any) {
    ...
    this.vendorService.PostPurchasedetail(newPurchase).subscribe((data) => {
      console.log(data);
      

    }, (err) => console.log(err));
    this.dialog.open(VendorPurchaseDetailComponent,{
